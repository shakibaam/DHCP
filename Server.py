...
class Broker():
    ...
    def talkToClient(self, xid,mac,addrss):
        ...
        if xid not in self.connected_clients_list:
            ...
            if reserve:
                ...
                if occupy_ip_len == all_ip_number:
                    ...
                    self.sock.sendto(string.encode(), addrss)
                else:
                    flag = True
                    offer = 0
                    offer_ip = ""
    ...
    def listen_clients(self):
        while True:
            ...
            if "DHCPDISCOVER" in msg_type["dhcp_message_type"]:
                ...
                t = threading.Thread(target=self.talkToClient, args=(client_xid,client_mac,client,))
    ...
    def buildPacket_offer(self,offer_ip,xid):
        ...
        serverip=bytes(map(int, str(self.serverIP).split('.')))
